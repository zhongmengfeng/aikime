...
public class ColorProfile {
	...
	private ColorProfile(String name) {
		...
		if (NIGHT.equals(name)) {
			...
			FillModeOption = new ZLEnumOption<ZLPaintContext.FillMode>("Colors", name + ":FillMode", ZLPaintContext.FillMode.fullscreen);
			...
			FooterNGForegroundOption = createOption(name, "FooterNGForegroundOption", 255, 255, 255);
...
//				new ZLStringOption("Colors", name + ":Wallpaper", "wallpapers/sepia.jpg");
			//此处将上面两行屏蔽，并重写如下，实现了去除墙纸，使得页面背景为纯白的效果
			WallpaperOption =
				new ZLStringOption("Colors", name + ":Wallpaper", "");
			FillModeOption =
				new ZLEnumOption<ZLPaintContext.FillMode>("Colors", name + ":FillMode", ZLPaintContext.FillMode.tile);
