...
public abstract class Util implements UserRegistrationConstants {
	...
	public static boolean isOurLink(String url) {
		try {
			return Uri.parse(url).getHost().endsWith(".fbreader.com.chaojiyiji");
		} catch (Throwable t) {
			return false;
		}
	}
	...
	public static Uri rewriteUri(Uri uri) {
		...
		if ("http".equals(uri.getScheme()) &&
			...
			if (bookId != null && !"".equals(bookId)) {
				return Uri.parse("litres-book://data.fbreader.com.chaojiyiji/catalogs/litres2/full.php5?id=" + bookId);
			}
		}
		return uri;
	}
