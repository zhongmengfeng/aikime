 ...
 *
 * @author xiaanming
 *         作者博客：http://blog.csdn.net/xiaanming/article/details/10298163
 */
public class RoundProgressBar extends View {
    ...
    public RoundProgressBar(Context context, AttributeSet attrs, int defStyle) {
        ...
        //最大进度
        maxpro = mTypedArray.getInteger(R.styleable.RoundProgressBar_maxpro, 100);
        //当前进度
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (style) {
            case STROKE: {
                 ...
                 *
                */
                canvas.drawArc(oval, startAngle, 360 * progress / maxpro, false, paint);  //根据进度画圆弧
                break;
            }
            case FILL: {
                paint.setStyle(Paint.Style.FILL_AND_STROKE);
                if (progress != 0)
                    canvas.drawArc(oval, startAngle, 360 * progress / maxpro, true, paint);  //根据进度画圆弧
                break;
            }
        }
        ...
        int percent = (int) (((float) progress / (float) maxpro) * 100);  //中间的进度百分比，先转换成float在进行除法运算，不然都为0
     ...
     * @param maxpro
     */
    public synchronized void setMax(int maxpro) {
        if (maxpro < 0) {
