...
import com.ichi2yiji.anki.exception.StorageAccessException;
import com.ichi2yiji.libanki.Collection;
import com.ichi2yiji.libanki.Storage;
...
public class CollectionHelper {
    ...
    /**
     * Prevents {@link com.ichi2yiji.async.CollectionLoader} from spuriously re-opening the {@link Collection}.
     *
     ...
     * This method is typically used when no Context is available to access the setting for the mCol path.
...
//        Log.e("dir.exists()","initializeAnkiDroidDirectory>>>>>>>>>>>>>>>>>>" + String.valueOf(dir.exists()));
//        Log.e("dir.mkdirs()","initializeAnkiDroidDirectory>>>>>>>>>>>>>>>>>>" + String.valueOf(dir.mkdirs()));
...
//                throw new StorageAccessException("Failed to create .nomedia file");
        ...
        //*********************** ADD BOOKS DIR *******************************//
        String temp = path + "/dirBooks";
        File dirBooks = new File(temp);
        if (!dirBooks.exists() && !dirBooks.mkdirs()) {
//            throw new StorageAccessException("Failed to create dirBooks directory");
        }
        temp = path + "/dirDecks";
        File dirDecks = new File(temp);
        if (!dirDecks.exists() && !dirDecks.mkdirs()) {
//            throw new StorageAccessException("Failed to create dirDecks directory");
        }

        temp = path + "/downloadBooks";
        File downloadBooks = new File(temp);


       if (!downloadBooks.exists() && !downloadBooks.mkdirs()) {
//            throw new StorageAccessException("Failed to create downloadBooks directory");
        }


        temp = path + "/dirTests";
        File dirTests = new File(temp);
        if (!dirTests.exists() && !dirTests.mkdirs()) {
//            throw new StorageAccessException("Failed to create dirTests directory");
        }

        temp = path + "/downloadTests";
        File downloadTests = new File(temp);
        if (!downloadTests.exists() && !downloadTests.mkdirs()) {
//            throw new StorageAccessException("Failed to create downloadTests directory");
        }

        temp = path + "/downloadDecks";
        File downloadDecks = new File(temp);
        if (!downloadDecks.exists() && !downloadDecks.mkdirs()) {
//            throw new StorageAccessException("Failed to create downloadDecks directory");
        }
        //*********************** ADD BOOKS DIR *******************************//
    }
    ...
    public static String getDefaultAnkiDroidDirectory() {
        return new File(Environment.getExternalStorageDirectory(), "Chaojiyiji").getAbsolutePath();
    }
