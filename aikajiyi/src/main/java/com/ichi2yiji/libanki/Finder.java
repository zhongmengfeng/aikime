...
public class Finder {
    ...
    private String _findAgainCards(String val) {
        val = val.substring(val.lastIndexOf(">") + 1);
        String format = String.format("c.id in (select cid from (select cid,count() from revlog where ease=1 group by cid having count()>=%s))", val);
        return format;
    }
